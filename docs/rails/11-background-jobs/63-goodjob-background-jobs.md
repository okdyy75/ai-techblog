---
title: "RailsとGoodJob: PostgreSQLベースのバックグラウンドジョブプロセッサ"
description: "Railsアプリケーションで、時間のかかる処理（メール送信、画像処理、外部APIへのリクエストなど）をユーザーのリクエストサイクルから切り離して非同期に実行するためには、バックグラウンドジョブの仕組みが不可欠です。"
---
